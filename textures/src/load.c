...
static int		read_deco(int i, int j, char **split_line, t_mlx *mlx)
{
	printf("j: %d\n", j);
	printf("i: %d\n", i);
	mlx->sectors[j].decos[i].wall_num = atof(split_line[1]);
	mlx->sectors[j].decos[i].tex = atof(split_line[2]);
	mlx->sectors[j].decos[i].origin_offset = ft_atoi(split_line[3]);
	mlx->sectors[j].decos[i].end_offset = ft_atoi(split_line[4]);
	return (i++);
}
...
int			read_sector(int *k, struct xy *vert, char **split_line, t_mlx *mlx)
{
	...
	mlx->sectors[*k].wall = malloc(vertex_len * sizeof(mlx->sectors[*k].wall));
	mlx->sectors[*k].decos = malloc(ft_atoi(split_line[3]) * sizeof(mlx->sectors[*k].decos));
	return (n);
}
...
void		load_data(t_mlx *mlx)
{
	...
	while (get_next_line(fd, &line) > 0)
	{
		...
		else if (split_line[0][0] == 's')
		{
			...
			l = read_deco(l, m, split_line, mlx);
